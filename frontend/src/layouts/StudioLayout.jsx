...
export default function StudioLayout() {
  ...
  const projectSearch =
    ["/queue", "/editor"].includes(location.pathname) &&
    new URLSearchParams(location.search).has("projectId")
      ? location.search
      : "";
    ...
    <div className="studio-shell">
      <div inert={sidebarOpen ? true : undefined}>
        ...
        <div className="studio-mobile-controls">
          <button
            ref={sidebarButton}
            className="icon-button studio-menu-toggle"
      ...
      >
        ...
        <nav aria-label="Navigasi studio">
          ...
          <NavLink to={`/queue${projectSearch}`} end onClick={closeSidebar}>
            <Icon name="clock" size={19} />
            Status Antrean
          </NavLink>
          <NavLink to={`/editor${projectSearch}`} end onClick={closeSidebar}>
        ...
        <div className="studio-sidebar-note">
          ...
          <strong>Satu konsep, satu cuplikan.</strong>
          <p>Jaga pengetahuan tetap utuh, dalam format yang mudah dibagikan.</p>
        </div>
        <div className="studio-account" ref={accountRef}>
          <button
            ...
            aria-controls="studio-account-panel"
            onClick={() => setAccountOpen(!accountOpen)}
          >
            <span className="studio-avatar">
              <Icon name="user" size={19} />
            </span>
            <span className="studio-account-identity">
          ...
          {accountOpen && (
            <div className="studio-account-panel" id="studio-account-panel">
              ...
              <p>{user.email}</p>
              <Link to="/">
                Kembali ke beranda <Icon name="arrow" size={14} />
              </Link>
              <button onClick={logout} disabled={leaving}>
                {leaving ? "Sedang keluar…" : "Keluar dari akun"}
              </button>
              {error && (
                <p className="studio-account-error" role="alert">
                  {error}
                </p>
              )}
            </div>
